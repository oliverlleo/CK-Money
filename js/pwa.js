...
function createPwaBanner() {
    ...
    // Criamos o banner oculto por padrão (display: none)
    const bannerHtml = \`
    ...
    // Inicializa os eventos dos botões logo após criar o HTML
    ...
    if (btnAccept) {
        btnAccept.addEventListener('click', async () => {
            // Só executa se tivermos o prompt salvo
            if (deferredPrompt) {
                // Oculta o banner
...
// Escuta o evento (Android/Chrome/Edge) para capturar o prompt de instalação
window.addEventListener('beforeinstallprompt', (e) => {
    // Previne a exibição da mini-infobar automática em dispositivos móveis
    e.preventDefault();

    // Salva o evento para dispararmos depois
    deferredPrompt = e;
    console.log('beforeinstallprompt disparado e capturado!');

    // Agora sim, já que a instalação está disponível, mostramos o banner
    const installBanner = document.getElementById('pwaInstallBanner');
    if (installBanner) {
        // Exibe o banner
...
// Quando o DOM estiver pronto, cria o HTML do banner (que começa oculto)
